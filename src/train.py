...
def ft_test(cjk_config, test_loader, num_nodes, relation_cnt, config, task_name='', quiet=False):
    ...
    AP_50 = []
    from tasks.real_query import cjk_FOLO_reasoning
    cjk_downstream_task = cjk_FOLO_reasoning(cjk_config[0],relation_cnt=cjk_config[1],
...
class TrainClient:
    def __init__(self, config, num_nodes, relation_cnt):
        ...
        if config.get('upstream_task_name') is not None:
            self.load_path = Path('pretrained_model/FOLO_camera_pretrain2.pt') # (Path(config['root_dir']) / 'chkpt' / config['upstream_task_name']).with_suffix('.pt')
        else:
            self.load_path = None
        self.save_path = Path('pretrained_model/FOLO_camera_pretrain2.pt') # (Path(config['root_dir']) / 'chkpt' / config['downstream_task_name']).with_suffix('.pt')
        self.backup_path = 'pretrained_model/FOLO_camera_pretrain2' # f'{config["root_dir"]}/chkpt/{config["downstream_task_name"]}'
